// A.Kuehl GEOG 575 activity 4 javascrpt main_with_debug.js
...
function cities(){
    ...
    for (var i = 0; i < cityPop.length; i++){
		...
		//calling three functions that are defined below.
    addColumns(cityPop);
    addEvents();
		// adding the additional debugCallBack function
		debugCallback();
};
...
// -----start of lab activity 4 ------------
//define callback function
function debugCallback(response){
	// define mydata for jquery append method below
	mydata = response;
	// jquery slects the div and appends each row of JSON data that is contained within the variable mydata to the div
	// the JSON.stringify converts the data contained in mydata into a string.
	$(mydiv).append(JSON.stringify(mydata));
};

// define debugAjax function
function debugAjax(){
	// creating a mydata variable to hold the cities JSON data
	var mydata;
	// jquery selets the JSON data from the link in the file
	$.ajax("data/MegaCities.geojson", {
		dataType: "json",
		success: function(response){
			// assigning the response of the jquery to the mydata variable
			mydata = response;
			// passing data to the debugCallback function after the success state is reached
			debugCallback(response);
		}
	});
	// jquery selects the div and adds a line break and the following title
	// the additional part of this jquery statement was deleted because of replication
	$(mydiv).append('<br>GeoJSON data:</br>');
};

//commenting out the additional statement below. Unecessary for the script because of the way it is set up above
// $(mydiv).append('GeoJSON data: ' + JSON.stringify(mydata));


//call the initialize function and debugAjax function when the document has loaded
